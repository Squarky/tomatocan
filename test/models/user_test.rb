...
[:email, :name, :permalink, :password ].each do |field|
  test "#{field.to_s}_must_not_be_empty" do
    ...
    #puts "User error test field_must_not_be_empty:  " + user.errors[field].to_s
  end
end
...
test "password and password_confirmation must match" do
  ...
  #puts "User error test PW&PWConfMustMatch:  " + user.errors[:password_confirmation].to_s
end
